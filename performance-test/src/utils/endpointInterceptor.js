...
const gauge = new prometheusClient.Gauge({
  name: `${name.replace('-','_')}_inprogress_requests`,
...
function onStart (req, res, { TRACE_HEADERS }) {
  ...
  req.span = span
  logger.info('recieved request of', `method: ${method}`, `url: ${originalUrl}`)
...
function onEnd (req, res) {
  ...
  if (statusCode >= 400) {
    ...
    logger.info(`completed the request with status: ${statusCode}`)
